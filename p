...
local function findID(namePet, typePet)
	for petName, petData in pairs(PetInventory) do
		if namePet == petData.id then
			...
			if Vrs == typePet and petData._am and petData._am >= 10 then
				return petName
			end
		end
	end
...
local function findAmount(namePet, typePet)
	for petName, petData in pairs(PetInventory) do
		if namePet == petData.id then
			local Vrs = (petData.pt == 1 and "Golden ") or (petData.pt == 2 and "Rainbow ") or "Normal"
			if Vrs == typePet and petData._am and petData._am >= 10 then
...
tabMachines:addToggle("Auto Machine", "z","big", false, function(value) 
	...
	local function processPets(pets, Mode)
		for _, pet in pairs(pets) do
			...
			local amountPet = findAmount(pet, TypeP)
			local petID = findID(pet, TypeP)

			if amountPet then
				print(petID, amountPet)
				local args = {
					[1] = petID,
				...
				if Mode == "Both" then
					...
					if nearMachine then
						activateMachine(nearMachine, args)
					end
